...
export default {
  ...
  computed: {
    ...
    iconUrl() {
      return this.weatherData
        ? 
`https://openweathermap.org/img/wn/$%7Bthis.weatherData.weather%5B0%5D.icon%7D.png%60`
        : null;
    },
  },
  mounted() {
  ...
  methods: {
    ...
    async fetchForecast(city) {
      const urlcast =
 `http://api.openweathermap.org/data/2.5/forecast?q=$%7Bcity%7D&appid=$%7Bapikey%7D%60`;
      try {
    ...
    async searchByCity() {
      ...
      try {
        const urlsearch = 
`http://api.openweathermap.org/data/2.5/weather?q=$%7Bthis.city%7D&appid=$%7Bapikey%7D%60`;
